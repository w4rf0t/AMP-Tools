...
def get_ip_nmap(target,status_data):
    ...
    threads = []
    for ip in IPs:
        threads.append(threading.Thread(target=os.system(f"nmap -sT -sV --top-ports 1000  {ip} >> Result/{target}/recon/{target}_ip/nmap_{ip}.txt")))
    ...
    data = []
    for ip in IPs:
        with open(f"Result/{target}/recon/{target}_ip/nmap_{ip}.txt","r") as file3:
            ...
            for match in matches:
                ...
                data.append(object)
    with open(f"Result/{target}/recon/{target}_ip/nmap_{target}.json","a") as file4:
