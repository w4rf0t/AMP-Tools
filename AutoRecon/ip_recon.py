...
import codecs
import unicodedata
from AutoRecon.module.zoomeye import zoomeye
...
def get_ip_nmap(target,status_data):
    ...
    threads = []
    for ip in IPs:
        threads.append(threading.Thread(target=zoomeye, args=(target,ip)))
                ...
                for data in data_json['matches']:
                    ...
                    try:
                        if title is not None and len(title) > 0:
                            title[0] = codecs.decode(title[0], 'unicode_escape')
                            title[0] = title[0].encode('latin-1').decode('utf-8')
                    except:
                        pass
                    pretty_json = {"ip":ip,"port":data['portinfo']['port'],"title":title,"service":data['portinfo']['service'],"app":data['portinfo']['app'],"extrainfo":data['portinfo']['extrainfo'],"version":data['portinfo']['version']}
                    #add json to json_data
                    json_data.append(pretty_json)
                    if pretty_json["service"] == "http" or pretty_json["service"] == "https":
                        file2.write(pretty_json["ip"]+":"+str(pretty_json["port"])+"\n")
    ...
    subprocess.run(f"rm -rf Result/{target}/recon/{target}_ip/zoomeye_*.json",shell=True)
    
    os.system(f"cat Result/{target}/recon/zoomeye_subdomain_{target}.txt >> Result/{target}/recon/final_subdomain_{target}.txt")
    command_probe = [f"cat Result/{target}/recon/zoomeye_subdomain_{target}.txt | ~/go/bin/httprobe >>  Result/{target}/recon/{target}_live.txt"]
...
def scan_input_IP(target,status_data):
    ...
    with open(f"Result/{target}/status_of_function.json","w") as f:
        json.dump(status_data, f, indent=4)
    subprocess.run(f"rm -rf Result/{target}/recon/{target}_ip/nmap_*.txt",shell=True)
    print(G,"Generating IP ports service done !")
def ip_Recon(target,status_data):
    ...
    subprocess.run(f"rm -rf Result/{target}/recon/zoomeye_subdomain_{target}.txt",shell=True)